...
namespace zed {
	...
	const char* TRANSFORMER_NAME = "mimic_transformer/v1.0";

	// g_MimicTransformer is stored as a pointer to delay construction until we've got arguments to construct with
	// (Since the ringbuffer has no default constructor, we can't store the actual class without a lot more hassle)
	unique_ptr<MimicTransformer> g_mimicTransformer;
	...
	{	
		...
		g_mimicTransformer = unique_ptr<MimicTransformer>(new MimicTransformer(bProp));
		spdlog::info("SignalProducerInitialize Ended");


		return g_mimicTransformer->init();

	}

	bool MimicTransformer::init() {
		...
		case zed::Zed4SignalType::Body_34_KeypointsPlus:
			...
			break;
		default:
			spdlog::debug("Mimic Transformer: Body Signal Type {} not currently implemented", int(bufferProperties.bodySignalType));
			return false;
			break;
		}
	...
	int MimicTransformer::get_data(uint8_t* mem, sig::time_point& time, sig::SignalMemory & sigMem) {
		int retval = 0;

		switch (bufferProperties.bodySignalType) {
		case zed::Zed4SignalType::Body_34_KeypointsPlus:
			retval = mimic_user<ZedSkeletonKPRot_34>(mem, time, sigMem);
			break;
		default:
			spdlog::debug("Mimic Transformer: Body Signal Type {} not currently implemented", int(bufferProperties.bodySignalType));
			retval = 0;
	...
	void MimicTransformer::set_local_idx(uint16_t idx) {
		local_idx = idx;
	...
	void MimicTransformer::shutdown() {
		switch (bufferProperties.bodySignalType) {
		case zed::Zed4SignalType::Body_34_KeypointsPlus:
	...
	DYNALO_EXPORT bool SignalTransformerInitializeConsumer(const sig::SignalProperties& sigProp, const sig::SignalConsumerRuntimeConfig& cfg) {
		...
		//g_mimicTransformer->initialize_signal(sigProp, cfg);
		return true;
	}
	...
	DYNALO_EXPORT int DYNALO_CALL GetSignalDataTransformer(uint8_t* mem, sig::time_point& time, sig::SignalMemory & sigMem) {		
		...
		int rv = g_mimicTransformer->get_data(mem, time, sigMem);	
	...
	DYNALO_EXPORT void DYNALO_CALL SetLocalUserIdx(uint16_t idx) {
		g_mimicTransformer->set_local_idx(idx);
	}
