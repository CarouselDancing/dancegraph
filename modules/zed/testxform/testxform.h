...
namespace zed {
    ...
    public:
		...
		template
		<typename Z>
		int nuke_rotations(ZedBodies<Z> &out_bodies, const ZedBodies<Z> &in_bodies_to, const ZedBodies<Z> &in_bodies_from, float scale_factor) {
			out_bodies.num_skeletons = in_bodies_to.num_skeletons;
			//out_bodies.num_skeletons = 1;
			//out_bodies.elapsed = sig::time_now();
			out_bodies.grab_delta = 0;
			out_bodies.track_delta = 0;
			out_bodies.process_delta = 0;
			
			spdlog::debug("Nuking rotations for {} skeletons, prior {}", in_bodies_to.num_skeletons, in_bodies_from.num_skeletons);
		...
		template
			...
			sig::SignalHistory * envHist = sigMem.get_history("env/v1.0");
			sig::SignalHistory * zedHist = sigMem.get_history("zed/v2.1");
			spdlog::info("History sizes: env: {}, zed: {}", envHist->queue_size(), zedHist->queue_size());
...

			{
				...
				signal_size = nuke_rotations(*zm, *zbLast, *zbPenult, time_factor);
				spdlog::debug("Outputting slerped rotation skel with {} bytes", signal_size);
